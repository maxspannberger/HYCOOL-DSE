...
# Import the global solve_system so we stay perfectly synced with main.py
from h2_components import Tank, Pipe, Pump, Corner, COOL, Valve, calc_frac, solve_system
...
if __name__ == "__main__":
    ...
    # Safely load the Temps generated by main.py
    try:
        filename_temps = "HEX_temps.json"
        with open(filename_temps, 'r') as f:
            All_temps = json.load(f)
    except FileNotFoundError:
        print("WARNING: HEX_temps.json not found. Run main.py first to establish baseline.")
        All_temps = {}

    # Safely load the physical areas generated by main.py
    try:
        filename_areas = "HEX_areas.json"
        with open(filename_areas, 'r') as f:
            HEX_areas = json.load(f)
    except FileNotFoundError:
        print("WARNING: HEX_areas.json not found. Run main.py first to establish geometry.")
        HEX_areas = None
    ...
    for current_phase, current_mdot in zip(c.oei_phases, c.oei_m_dots):
        ...
        # 4. Execute Simulation using global solve_system
        print(f"\n[Working Wing] Resolving states...")
        states_W, final_mdot_W, _, Temps_W = solve_system(system_W, m_dot=current_mdot, T_amb=c.T_amb)
        print(f"\n[Failed Wing] Resolving states...")
        states_F, final_mdot_F, _, Temps_F = solve_system(system_F, m_dot=current_mdot, T_amb=c.T_amb)
    ...
    # Re-save the Temps JSON to include the new OEI additions
